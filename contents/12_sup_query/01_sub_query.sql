...
1. 개념
  ...
  - 서브쿼리는 반드시 '( )'안에 기술 
  - 연산자와 함께 사용하는 경우 연산자 우측에 기술해야한다.
  - FROM절에 사용되는 서브쿼리를 특별히 in-line 서브쿼리라 하고  VIEW나 독립된
    테이블처럼 활용되며 실행 가능해야한다. // FROM절이 제일 먼저 실행되기 때문에 그것의 결과를 가지고 실행될것이다.

2. 메인쿼리와의 연관성 여부에 따라
  - 메인쿼리에 사용된 테이블과 서브쿼리에 사용된 테이블이 JOIN연산되었는지 여부에따라
  - 연관성 서브쿼리/연관성없는 서브쿼리
...
4. 반환되는 행과 열에 따라 
  - 단일행/단일열, 단일행/다중열, 다중행/단일열, 다중행/다중열 서브쿼리
  - 사용되는 연산자에 따라 구분

  
1) 연관성 없는 서브쿼리  
  - 메인쿼리에 사용된 테이블과 서브쿼리에 사용된 테이블이 JOIN연산이 발생되지 않는 서브쿼리

예) 사원테이블에서 사원들의 평균 급여보다 많은 급여를 지급받는 사원수를 조회하시오
...
예) 부서테이블의 부서의 관리자 사원번호가 100인 부서에 속한 사원수를 조회하시오
(메인쿼리 - 사원수)
...
예) 상품테이블에서 분류코드가 P200 구간에 속한 상품의 수를 조회하시오
(서브쿼리 사용하지 않는 쿼리)
...
예) 사원테이블에서 2006년 이후 입사한 사원들의 급여를 평균금여로 갱신하시오
...
예)평균급여보다 많은 급여를 받는 사원들을 삭제처리하시오
DELETE EMPLOYEES;
ROLLBACK;
...
--기본키의 개수만큼 where절
 ...
 (2)연관성 있는 서브쿼리
  - 메인쿼리에 사용된 테이블과 서브쿼리에 사용된 테이블이 join으로 연결된 서브쿼리
  
예) 장바구니테이블에서 최고 판매수량을 기록한 회원의 회원번호, 상품명, 수량을 조회하시오
...
예) 모든 거래처별 2005년 매출액합계를 구하시오
    Alias는 거래처코드, 거래처명, 매출액합계
...
예)장바구니테이블에서 2005년 4월에 판매된 상품 중 2005년6월에도 판매된 상품을 조회하시오 --동시에판매됨
...
예) 직무이력테이블(JOB_HISTORY) 테이블에 저장된 사원의 부서번호가
    사원테이블의 부서번호와 동일한 사원의 사원번호, 사원명, 부서명을 조회하시오
-- 동일부서에서 직무만 변경된사람
...
예)사원테이블에서 각 부서별 평균 급여보다 많은 급여를 받는 사원정보를 조회하시오
   Alias는 사원번호, 사원명, 부서번호, 급여이며 부서번호 순으로 출력하시오
...
예) 2005년도 제품별 매입수량합계와 매출수량합계를 조회하시오
...
예)2005년 1월 제품별 매입수량을 구하여 재고수불테이블(REMAIN)을 UPDATE하시오
