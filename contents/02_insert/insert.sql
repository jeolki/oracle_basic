...
INSERT INTO 테이블명[(컬럼명1,컬럼명2,...)]
 ...
 . '(컬럼명1,컬럼명2,...)'을 기술하는 경우 컬럼속성이 'NOT NULL'인 경우
   '컬럼명'을 생략할 수 없다(컬럼속성이 'NOT NULL'이 아닌 경우 '컬럼명'을 생략할 수 있으며
   해당 컬럼에 배정되는 '값'도 생략해야 한다.
 . '(컬럼명1,컬럼명2,...)'이 생략되면 테이블에 기술된 컬럼 순서와 갯수만큼 '값'을 기술해야한다.
 
 
 예)사원테이블에 다음 자료를 입력하시오
    [자료]
...
예) 사업장테이블(CONST_SITE)에 다음자료를 삽입하시오
[자료]
사업장번호 : W1001
사업장명 : 다살아신축아파트 공사
...
-- JOB_TITLE이 여러 테이블에 걸쳐있다. 한곳에서만 변경되면 업데이트어노말리가 발생 데이터의 일관성이 파괴될수있다.
...
SELECT * FROM WORK;  -- 어떤 데이터가 입력되었는지 확인
