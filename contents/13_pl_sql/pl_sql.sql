  ...
  - 익명불록, 사용자정의 함수, 저장프로시져, 트리거, 패키지 등이 제공
  
  
1. 익명블록(Anomymous Block)
  - 이름을 갖고 있지 않은 블록
  - PL/SQL의 기본이 되는 구조
  - 단순스크립트에서 실행되도 서버에 저장되지 않음
...
예) 년도를 입력 받아 윤년인지 평년인지를 판별하는 익명블록을 작성하시오
 DECLARE 
    V_YEAR NUMBER(4) := 0;
...
예)상품테이블에서 가장 큰 판매가를 가진 상품번호, 상품명, 분류명, 가격을 출력하는 프로시져를 작성하시오
