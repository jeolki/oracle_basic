 트리거(Trigger)
 - 임의의 이벤트 (INSERT, UPDATE, DELETE)가 발생되기 전 또는 발생 후 
 다른 테이블이 자동 변경이 되도록 하기 위해 사용되는 프로시져
 (사용형식)
...
1)타이밍
  - BEFORE와 AFTER
  - BEFORE : 이벤트가 발생되기전에 BEGIN ~ END 블록의 SQL문을 실행
...
3)WHEN 조건 : 행단위 트리거에서만 사용 가능(이벤트가 발생될때 구체적인 검색 조건 기술)
     -- 조건이 맞을 때만 트리거 동작
...
예) 상품코드 'p101000001' 상품을 고객 'c001' 고객이 '20050810'에
    10개를 구매했다고 가정하고 재고수불 테이블을 수정하는 트리거 작성
